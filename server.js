...
try {
const result = await pg.query(`SELECT u.id, u.name, u.email, u.plan, u.balance, u.role, u.created_at, COUNT(c.id) as campaign_count, COALESCE(SUM(c.spent), 0) as total_spent FROM users u LEFT JOIN campaigns c ON c.user_id = u.id WHERE u.role = 'customer' GROUP BY u.id ORDER BY u.created_at DESC`);
...
try {
const result = await pg.query(`SELECT c.*, u.name as user_name, u.email as user_email FROM campaigns c JOIN users u ON u.id = c.user_id ORDER BY c.created_at DESC`);
res.json(result.rows);
} catch (e) {
res.status(500).json({ error: “Server error” });
}
...
const session = await stripe.checkout.sessions.create({
  ...
  success_url: `${process.env.FRONTEND_URL || 'https://hmsmgroup.com'}?plan=success`,
  cancel_url: `${process.env.FRONTEND_URL || 'https://hmsmgroup.com'}?plan=cancelled`,
